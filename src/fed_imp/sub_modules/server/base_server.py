...
class ServerBase:
    ...
    def run(self):
        ...
        initial_values, sample_sizes, missing_ratios = [], [], []
        for client_id, client in self.clients.items():
            values, sample_size, ms_ratio = client.get_initial_values()
        ...
        ###############################################################################################
        aggregated_initial_values = self.strategy_imp.aggregate_initial(
            initial_values, sample_sizes, missing_ratios, self.client_groups
        )

        for client_id, client in self.clients.items():
            if isinstance(aggregated_initial_values, dict):
                client.initial_impute(aggregated_initial_values[client_id])
            else:
                client.initial_impute(aggregated_initial_values)
            # stats tracking
            if self.track:
        ...
        return {
            ...
            'imp_result': {
                ...
                'imp@sliced_ws': np.array([[value['imp@sliced_ws'] for value in item.values()] for item in imp_results]).mean(),
            },
            'pred_result': {
