...
class ServerCentral(ServerBase):

    def __init__(
            self,
            clients: Dict[int, Client], strategy_imp: StrategyImputation, server_config: dict,
            pred_config: dict, test_data: np.ndarray = None, seed: int = 21, base_model = 'mlp',  track=False,
        ...
        ###########################################################################
        # model
        self.pred_model_params = pred_config.get('model_params', {
            "num_hiddens": 64,
        })

        self.pred_training_params = pred_config.get('training_params', {
            "batch_size": 128,
            "learning_rate": 0.001,
            "weight_decay": 1e-4,
            "pred_round": 300
