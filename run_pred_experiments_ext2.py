...
def prediction(main_config, server_config_, round_idx, pred_rounds, seed, mtp=False):
...
if __name__ == '__main__':
    ...
    seed = 21
    mtp = True
    dir_name = 'fed_imp_ext2_pc2'
    datasets = ['1105/hhp_los_np1']
    mechanism = ['random2@mrl=0.3_mrr=0.7_mm=mnarlq']
    methods = ['fedavg-s', 'fedmechw_new', 'local']
    n_rounds = 5
    
    train_param = {"num_hiddens": 32, "batch_size": 300, "lr": 0.001, "weight_decay": 0.000, 'imbalance': None}
    ...
    for d in datasets:
        for m in mechanism:
            for method in methods:
                ...
                # read all files
                root_dir = f"./results/raw_results/{d}/{0}/natural/{m}/allk0.25_sphere/"
                print(root_dir)
                data_dir, exp_file = get_all_dirs(root_dir, method)
